...
//   // Confirm delete
//   const confirmDelete = () => {
//     alert(`Item deleted: ${itemToDelete}`);
  ...
  const fetchKeyword = async () => {
    ...
    if (token) {
      try {
        ...
        setData(response.data.data); // Update the data state with API response
      } catch (err) {
        console.error("Error fetching keywords:", err); // Log any errors to the console
      }
    } else {
      console.warn("No token found in localStorage."); // Log a warning if the token is missing
    }
  };
...
//   };
  ...
  const handleDeleteTask = async () => {
    ...
    axios
      .delete(`${import.meta.env.VITE_API_URL}/premium/deletelocation/${itemToDelete._id}`, {
        headers: { token },
      })
      .then((response) => {
