...
const FilterData = () => {
      // Example data
      const initialRows = [
            { cnrNumber: "12345", first: "John Doe", petitions: "Petition 1", advocates: "Advocate 1" },
            { cnrNumber: "67890", first: "Jane Smith", petitions: "Petition 2", advocates: "Advocate 2" },
            { cnrNumber: "11223", first: "Mark Johnson", petitions: "Petition 3", advocates: "Advocate 3" },
            { cnrNumber: "44556", first: "Emily Davis", petitions: "Petition 4", advocates: "Advocate 4" }
      ];
      ...
      const filteredRows = rows.filter((row) => row.cnrNumber.includes(searchQuery));

      const handleAddCnr = () => {
            if (newCnr) {
                  setRows([
                        ...rows,
                        { cnrNumber: newCnr, first: "New Name", petitions: "New Petition", advocates: "New Advocate" }
                  ]);
      ...
      return (
            <div className="mx-auto p-6 bg-white border border-gray-100 transition-all duration-300 max-w-full sm:max-w-4xl md:max-w-full">
                  <h2 className="text-2xl font-bold text-center text-gray-700 mb-6">My Council Filter</h2>
