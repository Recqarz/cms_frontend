...
const CaseTable = () => {
      ...
      const filteredData = data.filter((caseData) =>
            Object.values(caseData)
                  .join(" ")
                  .toLowerCase()
                  .includes(filterText.toLowerCase()) &&
            (filterOption ? caseData.status.toLowerCase() === filterOption.toLowerCase() : true)
      );
      ...
      const handleExport = () => {
            ...
            const csvHeader = ["CNR NUMBER", "FIRST HEARING", "NEXT HEARING", "PETITIONER", "RESPONDENT"];
            const csvRows = exportData.map((caseData) =>
                  [caseData.cnrNumber, caseData.firstHearing, caseData.nextHearing, caseData.petitioner, caseData.respondent].join(",")
            );
      ...
      return (
            <div className="shadow-md rounded-lg p-6">
                  <div>
                        <h1 className="text-2xl text-center mb-5 font-semibold">My Councils Case</h1>
                  </div>
                  ...
                  <div className="flex items-center mb-4 flex-wrap gap-4 justify-between">
                        ...
                        <div className="flex gap-x-3 flex-wrap sm:flex-nowrap w-full sm:w-auto">
                              ...
                              <button className="px-4 py-2 bg-purple-600 text-white rounded-md w-full sm:w-auto mt-2 sm:mt-0" onClick={toggleExportConfirm}>
                                    ...
                                    <tr className="bg-[#F4F2FF] text-left">
                                          {showCheckboxes && (
                                                <th className="py-2 px-4">
                                                      <input type="checkbox" checked={selectAll} onChange={handleSelectAll} className='w-full sm:w-1/4 h-full sm:h-5' />
                                                </th>
                                          )}
